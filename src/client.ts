...
import { AbstractPage, type ConversationsCursorParams, ConversationsCursorResponse } from './core/pagination';
...
export interface ClientOptions {
  /**
   * Defaults to process.env['INCONVO_API_KEY'].
   */
  apiKey?: string | undefined;

  /**
   * Override the API base URL via INCONVO_API_URL
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['INCONVO_LOG'] or 'warn' if it isn't set.
   */
...
export class Inconvo {
   ...
   * @param {string | undefined} [opts.baseURL=process.env['INCONVO_API_URL'] ?? undefined]
  ...
  constructor({
    ...
    baseURL = readEnv('INCONVO_API_URL'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
    ...
    if (baseURL === undefined) {
      throw new Errors.InconvoError(
        "The INCONVO_API_URL environment variable is missing or empty; either provide it, or instantiate the Inconvo client with an baseURL option, like new Inconvo({ baseURL: 'My Base URL' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? Inconvo.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('INCONVO_LOG'), "process.env['INCONVO_LOG']", this) ??
      defaultLogLevel;
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== 'https://app.inconvo.ai/api/v1';
  }
  ...
  >(
    ...
    return new Pagination.PagePromise<PageClass, Item>(this as any as Inconvo, request, Page);
  }
  ...
  static InconvoError = Errors.InconvoError;
